        ...
        <el-form ref="loginRef" :model="loginForm" :rules="loginRules">
          ...
          <el-form-item>
            <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
              <el-checkbox v-model="loginForm.rememberMe" style="color: darkgray; margin: 0;">
                记住密码
              </el-checkbox>
              <el-link 
                type="text" 
  ...
  proxy.$refs.loginRef.validate(valid => {
    if (valid) {
      ...
      if (loginForm.value.rememberMe) {
        Cookies.set("web_username", loginForm.value.username, { expires: 30 })
        Cookies.set("web_password", encrypt(loginForm.value.password), { expires: 30 })
        Cookies.set("web_rememberMe", loginForm.value.rememberMe, { expires: 30 })
      } else {
        // 否则移除
        Cookies.remove("web_username")
        Cookies.remove("web_password")
        Cookies.remove("web_rememberMe")
      }
      // 调用action的登录方法
...
function getCookie() {
  const username = Cookies.get("web_username")
  const password = Cookies.get("web_password")
  const rememberMe = Cookies.get("web_rememberMe")
  loginForm.value = {
