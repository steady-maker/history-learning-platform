        ...
        <pane size="84">
          <el-col>
            ...
            <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
              ...
              <el-table-column label="序号" align="center" key="index" v-if="columns.id.visible" width="120">
              ...
              <el-table-column label="手机号码" align="center" key="mobile" prop="mobile" v-if="columns.mobile.visible" />
              <el-table-column label="状态" align="center" key="status" v-if="columns.status.visible">
                <template #default="scope" :disabled="!checkPermi(['biz:webUser:edit'])">
                  <el-switch
              ...
              <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                <template #default="scope">
                  ...
                  <el-tooltip content="重置验证码次数" placement="top" v-if="scope.row.id !== 1">
                    <el-button link type="primary" icon="Key" @click="handleResetCode(scope.row)" v-hasPermi="['biz:webUser:reset_captcha']"></el-button>
                  </el-tooltip>
                </template>
              </el-table-column>
            </el-table>
...
<script setup name="WebUser">
import useAppStore from '@/store/modules/app'
import { changeUserStatus, listUser, delUser, getUser, updateUser, addUser, resetUserCode } from "@/api/biz/webUser"
...
function handleDelete(row) {
  if (row && row.id) {
    const index = userList.value.findIndex(item => item.id === row.id) + 1 // 找到行在列表中的索引
...
/** 重置密码按钮操作 */
function handleResetCode(row) {
    ElMessageBox.confirm('是否要重置"' + row.username + '"用户的验证码次数？',  '安全提示', {  confirmButtonText: '确定',  cancelButtonText: '取消',  type: 'warning' }).then(() => {
    resetUserCode({
      mobile: row.mobile
    }).then(() => {
