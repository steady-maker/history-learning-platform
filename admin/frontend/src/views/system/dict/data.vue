   ...
   <div class="app-container">
      ...
      <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
         ...
         <el-table-column label="字典标签" align="center" prop="dict_label">
            <template #default="scope">
               <span v-if="(scope.row.list_class == '' || scope.row.list_class == 'default') && (scope.row.css_class == '' || scope.row.css_class == null)">{{ scope.row.dict_label }}</span>
               <el-tag v-else :type="scope.row.list_class == 'primary' ? 'primary' : scope.row.list_class" :class="scope.row.css_class">{{ scope.row.dict_label }}</el-tag>
            </template>
         </el-table-column>
         ...
         <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
            <template #default="scope">
               ...
               <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:dict:rm']">删除</el-button>
            </template>
         </el-table-column>
      </el-table>
...
const data = reactive({
  ...
  rules: {
    dict_label: requiredAndTrim('数据标签不能为空'),
    dict_value: requiredAndTrim('数据键值不能为空'),
...
function getTypes(dictId) {
  getType(dictId).then(response => {
    queryParams.value.dict_type = response.data.type
...
function resetQuery() {
  ...
  queryParams.value.dict_type = defaultDictType.value
  handleQuery()
}
...
function handleDelete(row) {
  const dictCodes = row.dict_code || ids.value
