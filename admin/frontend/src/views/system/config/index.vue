   ...
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
         ...
         <el-form-item label="系统内置" prop="type">
            <el-select v-model="queryParams.type" placeholder="系统内置" clearable style="width: 240px">
               <el-option
      ...
      <el-table v-loading="loading" :data="configList" @selection-change="handleSelectionChange">
         ...
         <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
            <template #default="scope">
               ...
               <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:config:rm']">删除</el-button>
            </template>
         </el-table-column>
      </el-table>
...
const data = reactive({
  ...
  rules: {
    name: requiredAndTrim('参数名称不能为空'),
    key: requiredAndTrim('参数键名不能为空'),
    value: requiredAndTrim('参数键值不能为空')
  }
})
...
function handleDelete(row) {
  const config_ids = row.id || ids.value
  proxy.$modal.confirm('是否确认删除参数编号为"' + config_ids + '"的数据项？').then(function () {
