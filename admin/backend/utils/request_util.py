...
from system.models import LoginLog, LoginTypeEnum
...
def save_login_log(request, status="1", msg="", username=None):
    """
    保存登录日志
    :param request: 请求实例
    :param status: 登录状态 1:成功 0:失败
    :param msg: 登录日志备注
    :param username: 请求登录用户名
    """
    ip = get_request_ip(request=request)
    region = get_ip_region(ip)
    analysis_data = {'username': username or str(request.user.username), 'ip': ip,
                     'agent': str(parse(request.META['HTTP_USER_AGENT'])),
                     'browser': get_browser(request),'os': get_os(request),
                     'create_by': request.user.id, 'create_name': request.user.username,
                     'region': region, 'status': status, 'remark': msg, 'login_type': LoginTypeEnum.BACKEND}
